#!/usr/bin/env python3
"""
Probe triple-anchor DER families for destructive-conflict structure.

This follows frontier-12. Double-anchor DER templates stayed completely flat,
so the next question is whether planting the pushed-token anchor three times
inside one valid payload changes the FindAndDelete surface in a meaningful way.
"""
...
def make_triple_anchor_token(
    ...
    code = seg_a + anchor + seg_b + anchor + seg_c
...
def template_pool(spec: TemplateSpec, alphabet: tuple[int, ...]) -> tuple[bytes, ...]:
    return tuple(
        make_triple_anchor_token(spec.exact_len, bytes(seg_a), bytes(seg_b), bytes(seg_c))
        for seg_a in product(alphabet, repeat=spec.seg_a_len)
        for seg_b in product(alphabet, repeat=spec.seg_b_len)
        for seg_c in product(alphabet, repeat=spec.seg_c_len)
    )
...
def sampled_case(
    ...
    pool = template_pool(spec, alphabet)
    rng = random.Random(
        seed + spec.exact_len + spec.seg_a_len * 100 + spec.seg_b_len * 10 + spec.seg_c_len
    )
...
def main() -> None:
    exhaustive_specs = [
        (TemplateSpec("exact26_split1_1_1", exact_len=26, seg_a_len=1, seg_b_len=1, seg_c_len=1), (1, 2)),
        (TemplateSpec("exact27_split1_1_2", exact_len=27, seg_a_len=1, seg_b_len=1, seg_c_len=2), (1, 2)),
        (TemplateSpec("exact27_split1_2_1", exact_len=27, seg_a_len=1, seg_b_len=2, seg_c_len=1), (1, 2)),
        (TemplateSpec("exact27_split2_1_1", exact_len=27, seg_a_len=2, seg_b_len=1, seg_c_len=1), (1, 2)),
    ]
    ...
    sampled_specs = [
        (TemplateSpec("exact26_split1_1_1", exact_len=26, seg_a_len=1, seg_b_len=1, seg_c_len=1), (1, 2, 3)),
        (TemplateSpec("exact27_split1_1_2", exact_len=27, seg_a_len=1, seg_b_len=1, seg_c_len=2), (1, 2, 3)),
        (TemplateSpec("exact27_split1_2_1", exact_len=27, seg_a_len=1, seg_b_len=2, seg_c_len=1), (1, 2, 3)),
        (TemplateSpec("exact27_split2_1_1", exact_len=27, seg_a_len=2, seg_b_len=1, seg_c_len=1), (1, 2, 3)),
    ]
